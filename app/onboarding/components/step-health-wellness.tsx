...
import { ArrowRight, Activity, Target, Utensils, Ruler, Weight, Check } from "lucide-react";
import { useState, useEffect } from "react";
import { z } from "zod";

const healthWellnessSchema = z.object({
  height: z.number().min(120, "Height must be between 120 and 220 cm").max(220, "Height must be between 120 and 220 cm"),
  weight: z.number().min(30, "Weight must be between 30 and 200 kg").max(200, "Weight must be between 30 and 200 kg"),
...
export function StepHealthWellness() {
  ...
  const activityLevels = [
    ...
    { id: "athlete", label: "Extra Active", description: "Very hard exercise & physical job" },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4 p-6 rounded-2xl border bg-card/50">
            <div className="flex justify-between items-center">
              ...
              <span className="text-2xl font-bold text-primary">{formData.height} <span className="text-sm text-muted-foreground font-normal">cm</span></span>
            </div>
            <input
              type="range"
              min="120"
          ...
          <div className="space-y-4 p-6 rounded-2xl border bg-card/50">
            <div className="flex justify-between items-center">
              ...
              <span className="text-2xl font-bold text-primary">{formData.weight} <span className="text-sm text-muted-foreground font-normal">kg</span></span>
            </div>
            <input
              type="range"
              min="30"
            ...
            {activityLevels.map((level) => (
              <button
                ...
                className={cn(
                  ...
                  formData.activityLevel === level.id ? "border-primary bg-primary/5 ring-1 ring-primary" : "bg-card"
                )}
              >
            ...
            {goals.map((goal) => (
              <button
                ...
                className={cn(
                  ...
                  formData.primaryGoal === goal.id ? "border-primary bg-primary/5 ring-1 ring-primary text-primary font-medium" : "bg-card"
                )}
              >
            ...
            {dietaryPreferences.map((diet) => (
               <button
                ...
                className={cn(
                  ...
                  formData.dietaryPreference === diet.id ? "border-primary bg-primary/10 text-primary" : "bg-card"
                )}
              >
                {diet.label}
              </button>
