...
import { ArrowRight, ShieldCheck, FileText, Lock, Check } from "lucide-react";
import { useState } from "react";
import { z } from "zod";

const consentSchema = z.object({
  consent: z.literal(true, {
    errorMap: () => ({ message: "You must consent to continue using Aevio." }),
  }),
});
      ...
      <div className="space-y-6">
        <div className="grid gap-6 md:grid-cols-2">
            <div className="p-6 rounded-2xl border bg-card/50 space-y-4">
                ...
                <p className="text-sm text-muted-foreground">Your data is encrypted at rest and in transit. We use industry-standard security measures.</p>
            </div>
            <div className="p-6 rounded-2xl border bg-card/50 space-y-4">
                ...
                <p className="text-sm text-muted-foreground">You can export or delete your data at any time. We never sell your personal information.</p>
            </div>
        </div>
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        To provide you with personalized health insights, workout recommendations, and habit tracking analysis, we need to process the data you've shared.
                    </p>
                    <p className="text-sm text-muted-foreground mt-2">
                        By enabling this, you allow Aevio to analyze your inputs to generate tailored suggestions.
                    </p>
                </div>
            </div>
                 ...
                 <label className="flex items-center gap-4 cursor-pointer group">
                     <div className="relative flex items-center">
                        ...
                        <div className="w-6 h-6 border-2 border-muted-foreground rounded-md peer-checked:bg-primary peer-checked:border-primary transition-all flex items-center justify-center">
                            <Check className={cn("w-4 h-4 text-primary-foreground transition-transform scale-0", consent && "scale-100")} />
                        </div>
                     </div>
                     <div className="flex-1">
                         <span className={cn("font-medium transition-colors", consent ? "text-foreground" : "text-muted-foreground")}>
                            I consent to the processing of my health data for personalized insights.
                         </span>
                     </div>
                </label>
                 {error && <p className="text-sm text-destructive mt-2 ml-10">{error}</p>}
            </div>
        </div>

        <div className="text-center">
            <p className="text-xs text-muted-foreground">
                Read our <a href="#" className="underline hover:text-primary">Privacy Policy</a> and <a href="#" className="underline hover:text-primary">Terms of Service</a>.
            </p>
        </div>
      </div>
